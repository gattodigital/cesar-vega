...
@Component({
    ...
    // template: `<jqxGrid #gridReference [auto-create]='true'></jqxGrid>`      
})
...
export class PeopleComponent implements AfterViewInit {
  ...
  constructor(
    @Inject('peopleService') private _peopleService
  ) { }

 getAll() : Person[] {
    return this.people.map(p => p);
  }
  get(name: number) : Person {
    return this.people.find(p => p.name === name);
   
  }
  save(person: Person){
    let originalPerson = this.people.find(p => p.id === person.name);
  ...
  ngOnInit() {
   this._peopleService.search('t').subscribe(
  ...
  search2(value) {
    var randomQuote;
    this._peopleService.search2(value)
      .subscribe(
      data => this.people = data,
      err => this.logError(err),
      () => console.log('Random Quote Complete')
      ) // ...and calling .json() on the response to return data;
    // this.people = randomQuote;
  }
  logError(err) {
  ...
  // data: any = this._peopleService.generatedata(500, true);
  source: any =
  {
    // localdata: this.data,
    datatype: "array",
